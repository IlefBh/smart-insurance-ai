...
# Choose K (silhouette)
# =========================
def choose_k_by_silhouette(X_transformed, k_min: int = 2, k_max: int = 10) -> Tuple[int, Dict[int, float]]:
...
# =========================
# Deterministic naming + product recommendation
# =========================
def label_and_recommend(cluster: dict, global_medians: dict) -> Dict[str, Any]:
    """
    Adds business interpretation to clusters, deterministically and explainably.
    This is NOT pricing and NOT a black-box decision — just labeling + product orientation.
    """

    night_rate = _f(cluster["security_rates"].get("open_at_night", 0.0))
    alarm_rate = _f(cluster["security_rates"].get("security_alarm", 0.0))
    camera_rate = _f(cluster["security_rates"].get("security_camera", 0.0))
    ext_rate = _f(cluster["security_rates"].get("fire_extinguisher", 0.0))

    assets_med = _f(cluster["numeric_summary"]["assets_value_tnd"]["median"])
    years_med = _f(cluster["numeric_summary"]["years_active"]["median"])

    g_assets = _f(global_medians.get("assets_value_tnd", 0.0))
    g_years = _f(global_medians.get("years_active", 0.0))

    security_score = alarm_rate + camera_rate + ext_rate  # 0..3

    # Rule A: Night profile
    if night_rate >= 0.60 and security_score <= 1.20:
        return {
            "label": "Night & Cash – faible sécurité",
            "recommended_product": PRODUCT_NIGHT,
            "underwriting_flag": True,
            "risk_profile": "medium-high",
        }

    # Rule B: High assets exposure
    if g_assets > 0 and assets_med >= 1.25 * g_assets:
        return {
            "label": "Forte valeur exposée",
            "recommended_product": PRODUCT_PLUS,
            "underwriting_flag": False,
            "risk_profile": "medium",
        }

    # Rule C: New businesses (uncertainty)
    if g_years > 0 and years_med <= 0.60 * g_years:
        return {
            "label": "Jeune commerce – incertitude élevée",
            "recommended_product": PRODUCT_ESSENTIEL,
            "underwriting_flag": True,
            "risk_profile": "medium",
        }
    ...
    for cid in sorted(df_with_cluster["cluster_id"].unique()):
        ...
        for col in SEGMENTATION_CAT_COLS:
            ...
            cluster["categorical_summary"][col] = [
                {"value": str(k), "share": float(v)} for k, v in vc.items()
            ]

        # Top drivers (descriptive)
        drivers = []
        for col in SEGMENTATION_NUM_COLS:
            g = float(global_means.get(col, 0.0))
        ...
        # ✅ Add label + recommended_product
        cluster.update(label_and_recommend(cluster, global_medians))
...
# =========================
# Train segmentation and save artifacts
# =========================
def train_segmentation(df: pd.DataFrame, k_min: int = 2, k_max: int = 10) -> Tuple[int, Dict[int, float]]:
