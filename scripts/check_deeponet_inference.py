...
def build_t_from_df(df: pd.DataFrame) -> np.ndarray:
    # même logique que train: quantiles assets + revenue (2 dims) normalisés [0,1]
...
def main():
    ...
    # Load a few rows from dataset for realistic inference
    df = pd.read_csv(DATA_PATH)

    # 3 profils: "LOW-ish", "MEDIUM-ish", "HIGH-ish" (heuristique simple)
    # On ne change pas la data, on choisit juste des exemples existants.
    df_sorted = df.copy()
    df_sorted["risk_hint"] = (
        0.4 * df_sorted["open_at_night"].astype(float)
    ...
    # Build X and preprocess using same columns as training
    ...
    print("\nOK: if scores are in [0,1] and high-risk-hint tends to higher score, we're good.\n")
